...
public class Reponse {
  ...
  %include { term/Term.tom }
	 ...
	 * @param tree TomTree contenant la formule l'id de ses expression
	 * et des règles applicables sur celles-ci.
	 * @return String
	 */
	private String generateJsonRules(TomTree tree){
		String temp ="";
		Expr exp = null;
		...
		while(iterateur.hasNext())
		{
			...
			temp += "{\""+key +"\":[";
			exp = tree.getExprById(key);
			ArrayList<Pair<Expr,String>> liste = tree.getRuleById(key);
			if(liste.size() > 0){
				temp += ("{\"text\":\"$$"+ExprParser.exprToAlgebra(expr) + " => "+ExprParser.exprToAlgebra(liste.get(0).first)+"$$\","+"\"ruleId\":"+0+",\"type\":"+"\""+liste.get(0).second+"\"}");
			}
			for(int i = 1; i < liste.size(); i++){
				temp += ","+("{\"text\":\"$$"+ExprParser.exprToAlgebra(expr) + " => "+ExprParser.exprToAlgebra(liste.get(i).first)+"$$\","+"\"ruleId\":"+i+",\"type\":"+"\""+liste.get(i).second+"\"}");
			}
	 ...
	 * la formule courante.
	 * @param tree TomTree contenant la formule courante et les id de ses expression.
	 * @return String
	 */
	public String generateJsonId(TomTree tree){
		String temp ="[";
		Set<String> listKeys = tree.getExprIdsKeys();
	 ...
	 * @return String
	 */
	public String generateJsonExpressionList(ArrayList<Pair<Expr,Expr>> list){
		String temp ="[";
		if(list.size() > 0)
			temp += ("\"$$"+ list.get(0).first+"$$\"");
		for(int i = 1; i < list.size(); i++){
			temp += ",\"$$" + list.get(i).first+"$$\"";
		}
		return temp+"]";
	}
	 ...
	 * @param tree TomTree contenant l'historique des transformation de la
	 * formule courante.
	 * @return String
	 */
	public String getJsonTimeline (ArrayList<TomTree> trees) {
		String timeline = "";
		for (int i = 0 ; i < trees.size() ; i++) {
			String tmp = (String) ExprParser.exprToAlgebra(trees.get(i).getExprById(0));
			timeline += "{\"index\":" + i + ",";
			timeline += "\"text\":\"$$" + tmp +"$$\"}";
			if (i != trees.size() - 1)
				timeline += ",";
  ...
  public String formula (String gameId, String mode, int index) {
		...
		Expr resultat = tree.getExprById("0");

		"\\\\cssId{exp"+id +"}" +"{" +bexpression.firstExpression().generateExpression(id+"0") + operator.substring(1, operator.length()-1)+bexpression.secondExpression().generateExpression(id+"1")+ "}";
