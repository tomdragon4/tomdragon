...
public  class TomTree {
  ...
  %include { sl.tom }
  %include { expressions/expressions.tom }
...
/*
    ...
    */
      ...
      //++maxId;
  		...
  		//partie Tom
      %match(expr) {
        Nombre(a) -> {sousExprList.add(++maxId+"", `Nombre(a));}
        Litteral(x) -> {sousExprList.add(++maxId+"", `Litteral(x));}

        Plus(Litteral(a)) -> {sousExprList.add(++maxId+"", `Litteral(a));}
        Plus(Nombre(a)) -> {sousExprList.add(++maxId+"", `Nombre(a));}
        Plus(x, y) -> {
          sousExprList.add(++maxId+"", `Plus(x, y));
          generSousExpr(x);
          generSousExpr(y);
        }
        Plus(x, y, l1*) -> {
          sousExprList.add(++maxId+"", `Plus(x, Plus(y, l1*)));
          generSousExpr(x);
          generSousExpr(y);
          generSousExpr(`Plus(Plus(x,y), l1*));
        }

	Mult(Litteral(a)) -> {sousExprList.add(++maxId+"", `Litteral(a));}
        Mult(Nombre(a)) -> {sousExprList.add(++maxId+"", `Nombre(a));}
        Mult(x, y) -> {
        ...
        Parenthesis(x) -> {
          //Expr result_x = (Expr) `InnermostId(generRules()).visit(x);
          sousExprList.add(++maxId+"", `Parenthesis(result_x));
        }
      }
    ...
    */
    ...
    public void generRules (Expr expr, ArrayList<<Triplet<Expr, Expr, String>> variantes) {
    	//ArrayList<Pair<Expr, String>> variantes = new ArrayList<>();
    	//partie Tom
      %match(expr) {
        Nombre(a) -> {variantes.add(new Triplet(`Parenthesis(Nombre(a)), "contextMenu"));}
  	...
  	public void generAllRules (HashMap<String, Expr> expressions, HashMap<String,ArrayList<Triplet<Expr, Expr, String>>> rules) {
      ArrayList<Triplet<Expr, Expr, String>> generatedRules = new ArrayList<Triplet<Expr, Expr, String>>();
  		...
  		while(iterateur.hasNext()) {
  			key = iterateur.next();
  			this.generRules(expressions.get(key), generatedRules);
